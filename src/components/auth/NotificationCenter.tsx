  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <Bell size={18} />
        {unread > 0 && (
          <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-[#ff4d4f] text-[8px] font-bold text-white flex items-center justify-center">{unread > 9 ? '9+' : unread}</span>
        )}
      </button>
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-white/[0.04]">
              ...
              {notifications.length > 0 && (
                <button onClick={function() { clearNotifications(); }}
            ...
            <div className="max-h-[320px] overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-white/20">{isZh ? '暂无通知' : 'No notifications'}</p>
                </div>
              ) : (
                notifications.map(function(n) {
                  return (
                    <button key={n.id} onClick={function() { markNotificationRead(n.id); }}
